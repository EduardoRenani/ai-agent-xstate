...
| 3 | Agent State Machine -> LLM Client | in-process invoke (promise actor) | `{ messages: Array<{ role, content }>, systemPrompt: string, tools?: Record<string, Tool> }` |
| 4 | LLM Client -> OpenRouter API | HTTPS POST (sync) | `chat.completions.create` request |
